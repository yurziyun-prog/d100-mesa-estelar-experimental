...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'});try{
 ...
 await page.route('https://support.test/**',route=>{const file=new URL(route.request().url()).pathname.slice(1);return route.fulfill({contentType:file.endsWith('.js')?'text/javascript':'text/html',body:file.endsWith('.js')?fs.readFileSync(path.join(__dirname,'../js',file),'utf8'):'<html><head><style>'+[...fs.readFileSync(path.join(__dirname,'../index.html'),'utf8').matchAll(/<style[^>]*>([\s\S]*?)<\/style>/g)].map(m=>m[1]).join('\n')+'</style></head><body style="background:#19152c;color:white;font:16px sans-serif"><div id="novaSyncActionPanel"></div><div id="novaSyncActionParking"><button id="novaSyncNext">Passar</button></div></body></html>'});});
 ...
 await page.evaluate(async()=>{
  ...
  const info=()=>({firstAid:{value:100},health:health.actors.a.combateLab,targetHealth:health.actors.b.combateLab,kits:[{comp:'mochila',index:0,uses:inventory.mochila[0].usosRestantes}],inventory,powers:[{id:'teletransporte_caotico',name:'Teletransporte Caótico',cost:2,value:100},{id:'cura_psi',name:'Cura',cost:1,value:100,description:'1 PV por PP'}],psiMax:10,psiSpent:0});
  ...
  window.advance=async()=>{const round=map.combat.round;while(map.combat.round===round)map.combat=acaoIniciativa(map.combat,'spend',map.combat.turnId);await render();};window.refresh=render;window.inspect=()=>({health,map,inventory});await render();
 });
 ...
 await page.evaluate(()=>advance());await page.locator('#novaPsiTarget').selectOption('b');await page.locator('#novaPsiLocation').selectOption('Cabeça');await page.locator('#novaPsiCost').fill('2');await page.getByRole('button',{name:'Usar poder'}).click();state=await page.evaluate(()=>inspect());assert.equal(state.health.actors.a.psiSpent,2);assert.equal(state.health.actors.b.combateLab.hit.Cabeça,4);
