...
(async()=>{
 ...
 try {
  ...
  async function act(page,button){
    ...
    await page.waitForFunction(sel=>{const b=document.querySelector(sel);return !!b&&!b.disabled;},button);
  ...
  assert.ok(Math.abs(store[statePath+'/pj'].x-(origin.x-6))<.02,JSON.stringify({origin,saved:store[statePath+'/pj'],errors}));
  ...
  assert.equal(await master.locator('#novaSyncPersonagem').inputValue(),'__automatico__');
  assert.equal(await master.locator('#novaSyncNext').isVisible(),true);
